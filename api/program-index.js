...
async function catalogue(){
  ...
  for(const s of structures){
    ...
    const exact=byKey.get(s.key);
    const matches=exact?[exact]:(byIdentity.get(identity(s.university,s.programCode))||[]);
  ...
  cached={rows:eligible,totalPrograms:programs.length,totalStructures:structures.length};
  return cached;
}
export default async function handler(req,res){
  ...
  try{
    ...
    const query=norm(req.query?.q),subject=norm(req.query?.subject),university=norm(req.query?.university),coverage=norm(req.query?.coverage);
    ...
    return res.status(200).json({programs:rows,coverage:{complete:rows.length,partial:0,metadataOnly:0,total:rows.length,plannerVerified:rows.length,plannerUnprobed:0},catalogue:{uniquePrograms:data.rows.length,importedPrograms:data.totalPrograms,importedStructures:data.totalStructures},source:'studielots-ht26',fallback:false});
