...
function plannerCta(){const roots=[document.getElementById('myPathFlow'),document.getElementById('universityDetailContent')].filter(Boolean);for(const root of roots){for(const el of root.querySelectorAll('button,a')){const t=low(el.textContent);if((t.includes('planeraren')||t.includes('skapa plan'))&&!el.matches('[data-sl635-distance]'))return el}}return null}
...
window.__studielotsSharedPlanner={version:VERSION,sameDesign:true,distanceChronology:true,progressionAware:true,offeringAware:true,fastRouteShared:true,fastPaces:PACES.slice()};
})();
