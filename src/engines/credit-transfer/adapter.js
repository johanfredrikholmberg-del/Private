...
function applyToRows(rows,merits,options={}){
 ...
 const candidates=[];
 targets.forEach((row,ri)=>{if(row?.credited||isElective(row))return;sources.forEach(s=>{if(used.has(s.i)||!hp(s.merit)||!hp(row))return;const result=assess(s.merit,row,options);if(result?.classification==='strong')candidates.push({ri,s,result})})});
