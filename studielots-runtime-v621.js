(()=>{
  ...
  function installProgramSearch(){
    ...
    const render=()=>{
      const q=key(input.value),seen=new Set();
      const rows=(Array.isArray(window.programCatalog)?window.programCatalog:[]).filter(p=>active.has(key(p?.university))).filter(p=>{
        const code=key(p?.officialCode||p?.code);
        const id=key(p?.university)+'|'+(code||key(p?.name));
        if(seen.has(id))return false;seen.add(id);return !q||key((p?.name||'')+' '+(p?.university||'')+' '+(p?.officialCode||p?.code||'')).includes(q);
      ...
      host.innerHTML=rows.length?rows.map(p=>{
        ...
        return '<button type="button" class="program-search-row program-search-row-clickable" data-sl621-program="'+id+'"><div><strong>'+esc(p.name||'Program')+'</strong><span>'+esc(meta)+'</span></div><span>›</span></button>';
      }).join(''):'<div class="muted small program-empty">Inget program hittades.</div>';
      host.querySelectorAll('[data-sl621-program]').forEach(button=>button.addEventListener('click',event=>{
        event.preventDefault();event.stopPropagation();
        const id=decodeURIComponent(button.dataset.sl621Program||'');
        try{sessionStorage.setItem('lotsen_selected_program',id)}catch(_){}
        if(typeof window.selectProgramForMatch==='function')return window.selectProgramForMatch(id);
        if(typeof window.v390OpenProgram==='function')return window.v390OpenProgram(id);
        return window.v379ActivateProgram?.(id);
      },{once:true}));
    };
