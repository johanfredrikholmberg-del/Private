...
async function getJson(url){
  const r=await fetch(url,{headers:{accept:'application/json','user-agent':'StudieLots-GU-import/1.0'},redirect:'follow',signal:AbortSignal.timeout(25000)});
  if(!r.ok) throw new Error(`${r.status} ${r.statusText}`);
  return r.json();
}

function classify(data,item){
...
async function enrich(item){
  ...
  try{
    ...
    const c=classify(data,item);
    return {...item,status:c.coverage==='metadata-only'?'manual-review':'processed',...c,source:data?.source||'gu-official-programplan',sourceUrl:(data?.sourceUrls||[])[0]||'',sourceUrls:data?.sourceUrls||[],apiCoverage:data?.coverage||'',apiConfidence:data?.confidence||'',quality:data?.quality||{},checkedAt:new Date().toISOString()};
  }catch(e){return {...item,status:'manual-review',coverage:'metadata-only',reason:`gu-import:${e.message}`,checkedAt:new Date().toISOString()}}
}
...
async function main(){
  ...
  await fs.writeFile('data/gu/meta.json',JSON.stringify({generatedAt:new Date().toISOString(),programmes:gu.length,counts,unresolved:unresolved.length},null,2)+'\n');
  await fs.writeFile('data/gu/unresolved.json',JSON.stringify(unresolved,null,2)+'\n');
  console.log(JSON.stringify({programmes:gu.length,counts,unresolved:unresolved.length},null,2));
}
