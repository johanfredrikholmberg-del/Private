...
CODE_HP = re.compile(r'\b[A-Z]{1,5}\d[A-Z0-9]{2,6}\b.{0,110}?\b\d+(?:[.,]\d+)?\s*(?:hp|ECTS|credits)\b', re.I)
...
def main():
    ...
    session.headers['User-Agent'] = 'StudieLots educational source evidence collector/1.0'
    for name, url in SOURCES[institution]:
        ...
        try:
            ...
            # Candidate lines are raw evidence, NOT verified course records; never infer credits or classification.
            item['candidateTextLines'] = [line[:250] for line in lines if CODE_HP.search(line)][:200]
