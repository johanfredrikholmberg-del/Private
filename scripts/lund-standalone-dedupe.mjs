...
for(const x of incoming){
 ...
 if(canonicalProvider(x)!=='LU'||!code(x)||!Number.isFinite(points)||points<=0||!x.name||!x.sourceUrl){rejected.push({code:code(x),reason:'Saknar entydigt LU, kurskod, hp, namn eller källänk'});continue;}
...
console.log('Read-only: inga ändringar i den kanoniska databasen.');
