...
const INACTIVE=path.join(GU,'inactive-HT26.json');
...
let inactive=[];
try{inactive=JSON.parse(await fs.readFile(INACTIVE,'utf8'))}catch{}
const inactiveCodes=new Set(inactive.map(x=>codeNorm(typeof x==='string'?x:x.code)).filter(Boolean));

let structures=JSON.parse(await fs.readFile(TARGET,'utf8'));
const queue=JSON.parse(await fs.readFile(QUEUE,'utf8'));
const guQueueAll=queue.filter(x=>/goteborgs universitet/.test(norm(x.university)));
const inactiveKeys=new Set(guQueueAll.filter(x=>inactiveCodes.has(codeNorm(x.programCode))).map(x=>x.key));
const guQueue=guQueueAll.filter(x=>!inactiveKeys.has(x.key));
structures=structures.filter(x=>!inactiveKeys.has(x.key));

const byCode=new Map(structures.map((x,i)=>[codeNorm(x.programCode),i]));
let applied=0;
for(const o of overrides){
  if(inactiveCodes.has(codeNorm(o.programCode))) continue;
...
await fs.writeFile(path.join(GU,'meta.json'),JSON.stringify({database:'StudieLots HT26',generatedAt:new Date().toISOString(),programmes:guRows.length,counts,unresolved:unresolved.length,verifiedOverrides:applied,inactiveExcluded:inactiveKeys.size},null,2)+'\n');
await fs.writeFile(path.join(GU,'unresolved.json'),JSON.stringify(unresolved,null,2)+'\n');
console.log(JSON.stringify({applied,programmes:guRows.length,counts,unresolved:unresolved.length,inactiveExcluded:inactiveKeys.size},null,2));
