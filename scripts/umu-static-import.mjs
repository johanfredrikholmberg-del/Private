...
async function html(url){for(let i=0;i<5;i++){const r=await fetch(url,{headers:{accept:'text/html,application/xhtml+xml','user-agent':'StudieLots/1.0 data-quality import'},signal:AbortSignal.timeout(20000)});if(r.ok)return r.text();if(![429,500,502,503,504].includes(r.status)){const e=new Error(`${url} ${r.status}`);e.status=r.status;throw e}await sleep(1200*(i+1))}const e=new Error(`${url} temporarily unavailable`);e.temporary=true;throw e}
function strip(s){return String(s||'').replace(/<script[\s\S]*?<\/script>/gi,' ').replace(/<style[\s\S]*?<\/style>/gi,' ').replace(/<br\s*\/?>/gi,'\n').replace(/<\/(?:p|li|h[1-6]|div|tr|section|article|dt|dd)>/gi,'\n').replace(/<[^>]+>/g,' ').replace(/&nbsp;|&#160;|&#xA0;/gi,' ').replace(/&amp;/gi,'&').replace(/&aring;|&#229;/gi,'å').replace(/&auml;|&#228;/gi,'ä').replace(/&ouml;|&#246;/gi,'ö').replace(/&Aring;|&#197;/g,'Å').replace(/&Auml;|&#196;/g,'Ä').replace(/&Ouml;|&#214;/g,'Ö').replace(/[ \t]+/g,' ').replace(/\r/g,'').replace(/\n\s*\n+/g,'\n').trim()}
...
const structures=[];let temporaryErrors=0,officialFound=0,notFound=0;
for(let i=0;i<programmes.length;i++){
  ...
  }catch(e){if(e.status===404){notFound++;structures.push({...p,coverage:'catalogued-no-official-plan-page',reason:'official-education-plan-page-not-found',rows:[],sourceUrl:url,checkedAt:new Date().toISOString()})}else{temporaryErrors++;structures.push({...p,coverage:'temporarily-unavailable',reason:String(e.message||e),rows:[],sourceUrl:url,checkedAt:new Date().toISOString()})}}
...
await fs.writeFile('data/umu/meta.json',JSON.stringify({generatedAt:new Date().toISOString(),university:'Umeå universitet',programmes:programmes.length,courses:courses.length,officialFound,notFound,temporaryErrors,counts,policy:'SUSA supplies catalogue coverage. Programme structure is upgraded only from Umeå University official education-plan pages; unreconciled or unpublished terms remain explicit rather than guessed.'},null,2));
console.log(JSON.stringify({programmes:programmes.length,courses:courses.length,officialFound,notFound,temporaryErrors,counts},null,2));