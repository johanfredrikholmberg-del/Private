...
const years={
  1:[
    ['Mode och den kreativa processen',7.5],['Textil baskurs',5],['Fiber och garnteknologi',5],['Grundläggande marknadsföring',7.5],['Väveriteknik',5],['Redovisningens grunder och tekniker',7.5],['Trikåteknik',5],['Nonwoventeknik',5],['Organisering och ledning',7.5],['Konfektionsteknik',5]
  ],
  2:[
    ['Kvantitativ marknadsundersökningsmetodik',7.5],['Kvalitetssäkring och textil provning',5],['Färgning och beredning',5],['Logistik och styrning av försörjningskedjor',7.5],['Teorier om textil detaljhandel',7.5],['Modevetenskaplig teori och metod',7.5],['Affärssystem och organisering av kundrelationer',7.5],['Hållbarhet inom textil och mode',7.5],['Digital marknadsföring och modehandel',5]
  ],
  3:[
    ['Marknadskommunikation',7.5],['Konsumentbeteende',7.5],['Fältstudie inom detaljhandel',15],['Textilt management i ett detaljhandelsperspektiv',7.5],['Forskningsmetoder på kandidatnivå',7.5],['Självständigt arbete för kandidatexamen inom textilt management',15]
  ]
};
for(const code of ['SGKMM','SGKTM']){
  const x=all.find(v=>String(v.programCode||'').toUpperCase()===code);if(!x)continue;
  x.coverage='manual-review';
  x.reason='official-plan-verifies-academic-years-but-explicitly-not-chronological-within-years';
  x.rows=[];
  ...
  x.academicYearSums={1:60,2:60,3:60};
  x.studyStructureGranularity='academic-year-only';
  x.termPlacementVerified=false;
  x.sourceEvidenceUrl=official;
  x.verifiedProgrammeOverride=true;
  if(code==='SGKTM')x.successorProgrammeCode='SGKMM';
