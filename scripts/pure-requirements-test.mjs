...
assert(e?.version==='732','pure engine v732 loads');
...
assert(e.subjectMatch({subject:'Psykologi'},'Psykologi')===true,'exact subject matches');
assert(e.subjectMatch({subject:'Idrottsvetenskap'},'Vetenskap')===false,'substring inside another subject does not match');
assert(e.subjectMatch({subject:'Företagsekonomi'},'Ekonomi')===false,'partial suffix of another subject does not match');
assert(e.subjectMatch({subject:'Ekonomi och samhälle'},'Ekonomi')===true,'whole subject word in a compound field can match');
assert(e.subjectMatch({name:'Psykologi introduktion'},'Psykologi')===true,'course-name fallback matches a whole subject word');
assert(e.subjectMatch({name:'Neuropsykologi introduktion'},'Psykologi')===false,'course-name substring does not create a false subject match');
const courses=[
...
const missingSubject=[{name:'Statistik',hp:15,level:'G1N'},{name:'Psykologi introduktion',hp:15,level:'G1N'}];
