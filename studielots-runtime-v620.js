  ...
  function installProgramSearch(){
    ...
    const render=()=>{
      ...
      host.innerHTML=rows.length?rows.map(p=>{
        ...
        return '<button type="button" class="program-search-row program-search-row-clickable" data-sl620-program="'+id+'"><div><strong>'+esc(p.name||'Program')+'</strong><span>'+esc(meta)+'</span></div><span>›</span></button>';
      }).join(''):'<div class="muted small program-empty">Inget program hittades.</div>';
      host.querySelectorAll('[data-sl620-program]').forEach(button=>button.addEventListener('click',event=>{
        event.preventDefault();event.stopPropagation();
        const id=decodeURIComponent(button.dataset.sl620Program||'');
