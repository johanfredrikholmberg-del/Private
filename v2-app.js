...
function source(kind){state.source=kind;qa('[data-source]').forEach(b=>b.classList.toggle('active',b.dataset.source===kind));q('#sourceCopy').textContent=kind==='ladok'?'Ladda upp ett nationellt resultatintyg från Ladok. Din fil behandlas lokalt i webbläsaren.':'Ladda upp din merit-PDF från Antagning.se. Vi använder bara studieuppgifterna som behövs för matchningen.';}
function toggleTerm(btn){const row=btn.closest('.term-row'),list=row.querySelector('.course-list');list.hidden=!list.hidden;btn.querySelector('.chev').textContent=list.hidden?'⌄':'⌃';}
function fakeUpload(file){if(!file)return;const box=q('#uploadState');box.innerHTML=`<b>${file.name}</b><span>Redo att analyseras i StudieLots v2.</span>`;box.style.display='grid';}
...
q('#fastCta')?.addEventListener('click',()=>{show('planner');route('fast')});
