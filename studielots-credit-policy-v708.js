...
function historical(r){
  return !!(r && !official(r) && (
    r.historicalEvidence===true || r.historicalApproval===true ||
    r.historicalApproved===true || r.approvedHistorically===true ||
    r.previousApproved===true || Boolean(r.historicalDecision) ||
    (Array.isArray(r.historicalDecisionIds)&&r.historicalDecisionIds.length>0)
  ));
}
function inferred(r){
  if(!r || official(r) || historical(r)) return false;
  const st=low(r.status||r.matchStatus),mt=low(r.matchType);
  return !!(r.potentialCredit===true||r.credited===true||r.isCredited===true||r.creditApproved===true||num(r.matchedHp)>0||num(r.creditedHp)>0||mt.includes('replace')||st.includes('tillgodo')||st==='credited');
}
function candidateHp(r){return num(r?.potentialHp)||num(r?.historicalHp)||num(r?.matchedHp)||num(r?.creditedHp)||0}
function clearCreditState(out){
  out.credited=false;out.isCredited=false;out.creditApproved=false;out.matchedHp=0;out.creditedHp=0;out.historicalApproval=false;out.historicalApproved=false;out.approvedHistorically=false;out.previousApproved=false;
...
function sanitizeRow(r){
  ...
  const wasHistorical=historical(r),wasInferred=inferred(r);
  if(!wasHistorical&&!wasInferred)return r;
  const out={...r};
  ...
  if(wasHistorical){
    out.historicalEvidence=true;
    out.historicalHp=candidateHp(r);
    out.historicalReason=out.historicalReason||'Ett liknande tillgodoräknande har tidigare beviljats i ett historiskt universitetsbeslut.';
    out.potentialCredit=false;
    out.potentialHp=0;
    out.potentialReason='';
    out.matchType='historical';
  }else{
    out.potentialCredit=true;
    out.potentialHp=candidateHp(r);
    out.potentialReason=out.potentialReason||'StudieLots bedömer att tidigare studier kan motsvara kursen.';
    out.historicalEvidence=false;
...
function attachHistoricalEvidence(row,merits){
  ...
  if(!hits.length)return sanitizeRow(row);
  const unique=[...new Map(hits.map(h=>[h.decisionId,h])).values()];
  return sanitizeRow({...row,historicalEvidence:true,historicalHp:Math.min(hp(row),Math.max(...unique.map(h=>num(h.targetHp))))||0,historicalReason:'Liknande tillgodoräknande har tidigare beviljats. Detta är historisk evidens, inte ett beslut för dig.',historicalDecisionIds:unique.map(h=>h.decisionId),historicalSources:['Karlstads universitet'],potentialCredit:false,potentialHp:0});
}
...
function badgeText(r){if(r?.historicalEvidence)return'Tidigare bifall finns';if(r?.potentialCredit)return'Potentiellt tillgodoräknande';return''}
...
function annotateDom(){
  ...
  const rows=(Array.isArray(s?.plannerBaselineRows)?s.plannerBaselineRows:Array.isArray(s?.rows)?s.rows:Array.isArray(s?.courses)?s.courses:[]).filter(r=>r?.potentialCredit||r?.historicalEvidence);if(!rows.length)return;
  const nodes=document.querySelectorAll('.sl635-course,.sl-planner-course,.planner-course,.course-row');
  nodes.forEach(node=>{const text=norm(node.textContent),row=rows.find(r=>{const n=norm(name(r));return n&&text.includes(n)});if(!row)return;node.classList.remove('credited','partial');const host=node.querySelector('.sl635-course-copy')||node;let note=node.querySelector('.sl-potential-credit-note,.sl-historical-credit-note');if(!note){note=document.createElement('div');host.appendChild(note)}note.className=row.historicalEvidence?'sl-historical-credit-note':'sl-potential-credit-note';note.textContent=badgeText(row);let copy=node.querySelector('.sl-potential-credit-copy');if(!copy){copy=document.createElement('div');copy.className='sl-potential-credit-copy';host.appendChild(copy)}copy.textContent=row.historicalEvidence?'Ett liknande tillgodoräknande har tidigare beviljats. Kursen ligger ändå kvar att läsa tills du har ett eget beslut.':'StudieLots bedömer att kursen kan motsvara tidigare studier. Kursen ligger kvar att läsa och räknas inte av från återstående hp.'});
}
...
window.StudieLotsCreditPolicy={version:'708',official,historical,inferred,sanitizeRow,sanitizeRows,sanitizeSnapshot,importedMerits,exactHistoricalMatches,attachHistoricalEvidence,enrichRows,badgeText,countOfficial:rows=>sanitizeRows(rows).filter(official).reduce((s,r)=>s+(num(r.officialCreditHp)||hp(r)),0),countHistorical:rows=>sanitizeRows(rows).filter(historical).reduce((s,r)=>s+(num(r.historicalHp)||0),0),countPotential:rows=>sanitizeRows(rows).filter(r=>r?.potentialCredit&&!r?.historicalEvidence).reduce((s,r)=>s+(num(r.potentialHp)||0),0)};
...
window.dispatchEvent(new CustomEvent('studielots:credit-policy-ready',{detail:{version:'708'}}));
})();