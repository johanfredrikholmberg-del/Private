...
/* ---------- Direct degree/university -> planner transition ---------- */
let directPending=false,directTimer=0,directObserver=null;
const stopDirectObserver=()=>{if(directObserver){directObserver.disconnect();directObserver=null}};
...
function finishDirect(){if(!directPending)return true;const btn=plannerCta();if(!btn)return false;directPending=false;clearTimeout(directTimer);stopDirectObserver();btn.click();return true}
function beginDirect(){directPending=true;clearTimeout(directTimer);stopDirectObserver();document.documentElement.classList.add('sl633-direct-planner');directObserver=new MutationObserver(()=>{if(directPending&&finishDirect())setTimeout(()=>document.documentElement.classList.remove('sl633-direct-planner'),60)});directObserver.observe(document.getElementById('universityDetailContent')||document.body,{childList:true,subtree:true});let tries=0;const tick=()=>{if(finishDirect())return setTimeout(()=>document.documentElement.classList.remove('sl633-direct-planner'),60);if(++tries<16)return setTimeout(tick,25);directPending=false;stopDirectObserver();document.documentElement.classList.remove('sl633-direct-planner');window.openStudyPlanner?.()};setTimeout(tick,0);directTimer=setTimeout(()=>{directPending=false;stopDirectObserver();document.documentElement.classList.remove('sl633-direct-planner')},800)}

document.addEventListener('click',e=>{if(e.target.closest('.uni-path-btn')&&document.querySelector('.screen.active')?.id==='degreeDetail')beginDirect();if(e.target.closest('[data-screen="plannerClean"]'))setTimeout(()=>refreshPlanner(false),50)},true);
