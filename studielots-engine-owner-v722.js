...
const stats={version:VERSION,calls:0,comparable:0,matches:0,mismatches:0,errors:0,byEntry:{},deltas:[],last:null,lastError:null};
function remember(entry,detail){const bucket=stats.byEntry[entry]||(stats.byEntry[entry]={comparable:0,matches:0,mismatches:0,maxAbsDeltaHp:0});bucket.comparable++;if(detail.match)bucket.matches++;else bucket.mismatches++;bucket.maxAbsDeltaHp=Math.max(bucket.maxAbsDeltaHp,Math.abs(detail.deltaHp));stats.deltas.push(detail);if(stats.deltas.length>50)stats.deltas.shift();stats.last=detail;try{sessionStorage.setItem('studielots_engine_shadow_v726',JSON.stringify(detail));sessionStorage.setItem('studielots_engine_shadow_stats_v726',JSON.stringify({calls:stats.calls,comparable:stats.comparable,matches:stats.matches,mismatches:stats.mismatches,errors:stats.errors,byEntry:stats.byEntry}))}catch(_){}window.dispatchEvent(new CustomEvent('studielots:engine-shadow',{detail}))}
function shadow(name,args,result){stats.calls++;if(typeof pure.evaluateRequirements!=='function')return;const courses=findCourses(args),requirements=inferRequirements(args,result),legacy=legacyRemaining(result);if(!courses.length||!requirements||legacy==null)return;try{const next=pure.evaluateRequirements(courses,requirements),fresh=num(next?.remainingHp);if(fresh==null)return;const delta=Math.round((fresh-legacy)*10)/10,match=Math.abs(delta)<0.01;stats.comparable++;if(match)stats.matches++;else stats.mismatches++;remember(name,{at:new Date().toISOString(),entry:name,legacyRemainingHp:legacy,pureRemainingHp:fresh,deltaHp:delta,match,courseCount:courses.length,requirements:next.requirements,completed:next.completed,gaps:next.gaps})}catch(error){stats.errors++;stats.lastError={at:new Date().toISOString(),entry:name,message:String(error?.message||error)}}
}
...
window.__studielotsEngine=Object.freeze({
 ...
 getShadowReport:()=>JSON.parse(JSON.stringify(stats)),clearShadowReport:()=>{stats.calls=stats.comparable=stats.matches=stats.mismatches=stats.errors=0;stats.byEntry={};stats.deltas.length=0;stats.last=stats.lastError=null},
...
window.__studielotsBuild={...(window.__studielotsBuild||{}),engineOwner:VERSION,engineOwnership:'single-owner-shadow-diagnostics',pureEngineShadow:true};
