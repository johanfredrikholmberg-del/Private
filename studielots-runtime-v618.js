  ...
  function installProgramSearch(){
    ...
    const render=()=>{
      ...
      const rows=(Array.isArray(window.programCatalog)?window.programCatalog:[]).filter(p=>active.has(key(p?.university))).filter(p=>{
        const id=key(p?.university)+'|'+key(p?.name);
      ...
      host.innerHTML=rows.length?rows.map(p=>{
        ...
        return '<button type="button" class="program-search-row program-search-row-clickable" data-sl618-program="'+id+'"><div><strong>'+esc(p.name||'Program')+'</strong><span>'+esc(meta)+'</span></div><span>›</span></button>';
      }).join(''):'<div class="muted small program-empty">Inget program hittades.</div>';
      host.querySelectorAll('[data-sl618-program]').forEach(button=>button.addEventListener('click',event=>{
        event.preventDefault();event.stopPropagation();
        const id=decodeURIComponent(button.dataset.sl618Program||'');
        ...
        if(typeof window.v379ActivateProgram==='function')return window.v379ActivateProgram(id);
        return window.selectProgramForMatch?.(id);
      },{once:true}));
    };
