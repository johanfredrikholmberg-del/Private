(()=>{
'use strict';
const VERSION='653',CORE='/studielots-runtime-core-v641.js?v=641',UI='/studielots-planner-ui-v647.js?v=652',GUARD='/studielots-planner-guard-v653.js?v=653',SNAPSHOT='studielots_planner_snapshot',ORIGIN='studielots_planner_origin';
let busy=false;
...
function loadUi(){if(document.querySelector('script[data-studielots-planner-ui="652"]'))return;const u=document.createElement('script');u.src=UI;u.async=false;u.dataset.studielotsPlannerUi='652';document.body.appendChild(u)}
function loadGuard(){if(document.querySelector('script[data-studielots-planner-guard="653"]'))return;const g=document.createElement('script');g.src=GUARD;g.async=false;g.dataset.studielotsPlannerGuard='653';document.body.appendChild(g)}
function install(){document.addEventListener('click',schedule,true);window.addEventListener('studielots:planner-snapshot',schedule);window.addEventListener('studielots:screen-rendered',schedule);window.addEventListener('pageshow',schedule);if(!document.getElementById('sl645-style')){const st=document.createElement('style');st.id='sl645-style';st.textContent='.screen.sl645-directing{visibility:hidden!important}';document.head.appendChild(st)}window.__studielotsBuild={...(window.__studielotsBuild||{}),plannerOpenGuard:VERSION,obsoleteSummaryAuto:'enabled'};loadUi();loadGuard();schedule()}
