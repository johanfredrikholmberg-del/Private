...
function script(src,attrs={}){
  return new Promise(resolve=>{
    ...
    if(existing){loaded.push(src);resolve(true);return}
    const s=document.createElement('script');
    s.src=src;s.async=false;
    Object.entries(attrs).forEach(([k,v])=>s.dataset[k]=v);
    s.onload=()=>{loaded.push(src);resolve(true)};
    s.onerror=()=>{failed.push(src);console.error('StudieLots kunde inte ladda',src);resolve(false)};
    document.body.appendChild(s);
  });
}
function css(href){
  if(document.querySelector('link[data-studielots-responsive="706"]'))return;
